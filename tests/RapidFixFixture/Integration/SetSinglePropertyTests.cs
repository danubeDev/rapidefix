  ...
  public class SetSinglePropertyTests : IClassFixture<SetSinglePropertyTestsFixture>
  {
    ...
    public void MapAndSetInteger()
    {
      ...
      _fixture.CompositeSetter.Set(
        testTag.Value,
        _fixture.PropertyMapper.TryGet(testTag.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);
    ...
    [Fact]
    public void MapAndSetTypeConvertedProperty()
    {
      Parallel.For(0, 1000, i =>
        {
          var to = new TestTypeParent();
          var mo = new FixMessageContext();
          int testValue = 0199 + i;
          var testTag = new FixTagValue(61, testValue);
          ...
          if(testValue != to.Tag61.Value)
          {
            Console.Write(testValue);
          }
          Assert.Equal(testValue, to.Tag61.Value);
        });
    ...
    public void MapAndSetRepeatingGroup()
    {
      ...
      _fixture.CompositeSetter.Set(
        ...
        _fixture.PropertyMapper.TryGet(repeatingGroupTag.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);
    ...
    public void MapAndSetEnumeratedString()
    {
      ...
      _fixture.CompositeSetter.Set(
        testTag0.Value,
        _fixture.PropertyMapper.TryGet(testTag0.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);

      _fixture.CompositeSetter.Set(
        testTag1.Value,
        _fixture.PropertyMapper.TryGet(testTag1.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);
    ...
    public void MapAndSetAllEnumeratedChildTypeProperty()
    {
      ...
      _fixture.CompositeSetter.Set(
        testTag00.Value,
        _fixture.PropertyMapper.TryGet(testTag00.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);

      _fixture.CompositeSetter.Set(
        testTag01.Value,
        _fixture.PropertyMapper.TryGet(testTag01.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);

      _fixture.CompositeSetter.Set(
        testTag10.Value,
        _fixture.PropertyMapper.TryGet(testTag10.Tag.ToKnownTag()),
        MessageContext,
        TargetObject);

      _fixture.CompositeSetter.Set(
              testTag11.Value,
              _fixture.PropertyMapper.TryGet(testTag11.Tag.ToKnownTag()),
              MessageContext,
              TargetObject);
